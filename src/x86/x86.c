...
uint8_t x86_dec_k81(uint8_t* bin, uint64_t* bn, uint64_t* addr, uint8_t op0, uint8_t op1, int8_t* mn) {
	if (bin[*bn] == op0 && (bin[*bn + 1] >> 3) == (0 | op1)) {
		...
		*bn = *bn + 1;
		...
		uint8_t k = bin[*bn];
		*bn = *bn + 1;
		printf("      %sw %s, %u", mn, x86_r16(mrd), k);
...
uint8_t x86_dec_mvp(uint8_t* bin, uint64_t* bn, uint64_t* addr, uint8_t op, int8_t* mn) {
	if ((bin[*bn] & 248) == op) {
		printf("%02x ", bin[*bn]);
		uint8_t reg = bin[*bn] & 7;
		*bn = *bn + 1;
...
uint8_t x86_dec_alnk(uint8_t* bin, uint64_t* bn, uint64_t* addr, uint8_t op0, uint8_t op1, int8_t* mn) {
	if (bin[*bn] == op0 && (bin[*bn + 1] >> 3) == (0 | op1)) {
		printf("%02x ", bin[*bn]);
		*bn = *bn + 1;
		printf("%02x ", bin[*bn]);
		...
		*bn = *bn + 1;
		if (d & 128) {
			d = ~d + 1;
			printf("         %sb (%s, -%u)", mn, x86_a16(mrd), d);
		}
		else {
			printf("         %sb (%s, %u)", mn, x86_a16(mrd), d);
		}
		...
		*bn = *bn + 1;
		if (d & 128) {
			d = ~d + 1;
			printf("         %sw (%s, -%u)", mn, x86_a16(mrd), d);
		}
		else {
			printf("         %sw (%s, %u)", mn, x86_a16(mrd), d);
		}
...
uint8_t x86_dec_mlnk(uint8_t* bin, uint64_t* bn, uint64_t* addr, uint8_t op0, uint8_t op1, int8_t* mn) {
	if (bin[*bn] == op0 && (bin[*bn + 1] >> 3) == (0 | op1)) {
		printf("%02x ", bin[*bn]);
		*bn = *bn + 1;
		printf("%02x ", bin[*bn]);
		...
		*bn = *bn + 1;
		if (d & 128) {
			d = ~d + 1;
			printf("         %sb ax, (%s, -%u)", mn, x86_a16(mrd), d);
		}
		else {
			printf("         %sb ax, (%s, %u)", mn, x86_a16(mrd), d);
		}
		...
		*bn = *bn + 1;
		if (d & 128) {
			d = ~d + 1;
			printf("         %sw ax, (%s, -%u)", mn, x86_a16(mrd), d);
		}
		else {
			printf("         %sw ax, (%s, %u)", mn, x86_a16(mrd), d);
		}
...
uint8_t x86_dec_cond(uint8_t* bin, uint64_t* bn, uint64_t* addr, uint8_t op, int8_t* mn) {
	if (bin[*bn] == op) {
		printf("%02x ", bin[*bn]);
		*bn = *bn + 1;
...
uint8_t x86_dec_jmp_byt(uint8_t* bin, uint64_t* bn, uint64_t* addr, uint8_t op, int8_t* mn) {
	if (bin[*bn] == op) {
		printf("%02x ", bin[*bn]);
		*bn = *bn + 1;
		printf("            %02x ", bin[*bn]);
...
uint8_t x86_dec_shft_k80(uint8_t* bin, uint64_t* bn, uint64_t* addr, uint8_t op0, uint8_t op1, int8_t* mn) {
	if (bin[*bn] == op0 && (bin[*bn + 1] >> 3) == (0 | op1)) {
		printf("%02x ", bin[*bn]);
		*bn = *bn + 1;
		printf("%02x ", bin[*bn]);
		...
		*bn = *bn + 1;
		if (d & 128) {
			d = ~d + 1;
			printf("            %sb (%s, -%u), 1", mn, x86_a16(mrd), d);
		}
		else {
			printf("            %sb (%s, %u), 1", mn, x86_a16(mrd), d);
		}
...
uint8_t x86_dec_shft_k81(uint8_t* bin, uint64_t* bn, uint64_t* addr, uint8_t op0, uint8_t op1, int8_t* mn) {
	if (bin[*bn] == op0 && (bin[*bn + 1] >> 3) == (0 | op1)) {
		printf("%02x ", bin[*bn]);
		*bn = *bn + 1;
		printf("%02x ", bin[*bn]);
		...
		*bn = *bn + 1;
		if (d & 128) {
			d = ~d + 1;
			printf("            %sw (%s, -%u), 1", mn, x86_a16(mrd), d);
		}
		else {
			printf("            %sw (%s, %u), 1", mn, x86_a16(mrd), d);
		}
...
uint8_t x86_dec_shft_r8(uint8_t* bin, uint64_t* bn, uint64_t* addr, uint8_t op0, uint8_t op1, int8_t* mn) {
		...
		*bn = *bn + 1;
		...
		if (mod == 0) {
			...
			*bn = *bn + 1;
			if (d & 128) {
				d = ~d + 1;
				printf("            %sb (%s, -%u), cl", mn, x86_a16(mrd), d);
			}
			else {
				printf("            %sb (%s, %u), cl", mn, x86_a16(mrd), d);
			}
...
uint8_t x86_dec_shft_reg(uint8_t* bin, uint64_t* bn, uint64_t* addr, uint8_t op0, uint8_t op1, int8_t* mn) {
	if (bin[*bn] == op0 && ((bin[*bn + 1] >> 3) & 7) == op1) {
		printf("%02x ", bin[*bn]);
		*bn = *bn + 1;
		printf("%02x ", bin[*bn]);
		...
		if (mod == 0) {
			...
			*bn = *bn + 1;
			if (d & 128) {
				d = ~d + 1;
				printf("            %sw (%s, -%u), cl", mn, x86_a16(mrd), d);
			}
			else {
				printf("            %sw (%s, %u), cl", mn, x86_a16(mrd), d);
			}
...
void x86_dec(uint8_t* bin, uint64_t* bn, uint64_t* addr) {
	...
	if (eo) {
		eo = x86_dec_cond(bin, bn, addr, 112, "jo");
	}
	if (eo) {
		eo = x86_dec_cond(bin, bn, addr, 113, "jno");
	}
	if (eo) {
		eo = x86_dec_cond(bin, bn, addr, 114, "jc");
	}
	if (eo) {
		eo = x86_dec_cond(bin, bn, addr, 115, "jnc");
	}
	if (eo) {
		eo = x86_dec_cond(bin, bn, addr, 116, "je");
	}
	if (eo) {
		eo = x86_dec_cond(bin, bn, addr, 117, "jne");
	}
	if (eo) {
		eo = x86_dec_cond(bin, bn, addr, 118, "jna");
	}
	if (eo) {
		eo = x86_dec_cond(bin, bn, addr, 119, "ja");
	}
	if (eo) {
		eo = x86_dec_cond(bin, bn, addr, 120, "js");
	}
	if (eo) {
		eo = x86_dec_cond(bin, bn, addr, 121, "jns");
	}
	if (eo) {
		eo = x86_dec_cond(bin, bn, addr, 122, "jpe");
	}
	if (eo) {
		eo = x86_dec_cond(bin, bn, addr, 123, "jpo");
	}
	if (eo) {
		eo = x86_dec_cond(bin, bn, addr, 124, "jl");
	}
	if (eo) {
		eo = x86_dec_cond(bin, bn, addr, 125, "jge");
	}
	if (eo) {
		eo = x86_dec_cond(bin, bn, addr, 126, "jle");
	}
	if (eo) {
		eo = x86_dec_cond(bin, bn, addr, 127, "jg");
	}
	if (eo) {
	...
	if (eo) {
		eo = x86_dec_mvp(bin, bn, addr, 176, "mov");
	}
	if (eo) {
		eo = x86_dec_k80(bin, bn, addr, 192, 0, "rol");
	}
	if (eo) {
		eo = x86_dec_k80(bin, bn, addr, 192, 1, "ror");
	}
	if (eo) {
		eo = x86_dec_k80(bin, bn, addr, 192, 2, "rcl");
	}
	if (eo) {
		eo = x86_dec_k80(bin, bn, addr, 192, 3, "rcr");
	}
	if (eo) {
		eo = x86_dec_k80(bin, bn, addr, 192, 4, "shl");
	}
	if (eo) {
		eo = x86_dec_k80(bin, bn, addr, 192, 5, "shr");
	}
	if (eo) {
		eo = x86_dec_k80(bin, bn, addr, 192, 6, "sal");
	}
	if (eo) {
		eo = x86_dec_k80(bin, bn, addr, 192, 7, "sar");
	}
	if (eo) {
		eo = x86_dec_k81(bin, bn, addr, 193, 0, "rol");
	}
	if (eo) {
		eo = x86_dec_k81(bin, bn, addr, 193, 1, "ror");
	}
	if (eo) {
		eo = x86_dec_k81(bin, bn, addr, 193, 2, "rcl");
	}
	if (eo) {
		eo = x86_dec_k81(bin, bn, addr, 193, 3, "rcr");
	}
	if (eo) {
		eo = x86_dec_k81(bin, bn, addr, 193, 4, "shl");
	}
	if (eo) {
		eo = x86_dec_k81(bin, bn, addr, 193, 5, "shr");
	}
	if (eo) {
		eo = x86_dec_k81(bin, bn, addr, 193, 6, "sal");
	}
	if (eo) {
		eo = x86_dec_k81(bin, bn, addr, 193, 7, "sar");
	}
	if (eo) {
	...
	if (eo) {
		eo = x86_dec_shft_k80(bin, bn, addr, 208, 1, "ror");
	}
	if (eo) {
		eo = x86_dec_shft_k80(bin, bn, addr, 208, 2, "rcl");
	}
	if (eo) {
		eo = x86_dec_shft_k80(bin, bn, addr, 208, 3, "rcr");
	}
	if (eo) {
		eo = x86_dec_shft_k80(bin, bn, addr, 208, 4, "shl");
	}
	if (eo) {
		eo = x86_dec_shft_k80(bin, bn, addr, 208, 5, "shr");
	}
	if (eo) {
		eo = x86_dec_shft_k80(bin, bn, addr, 208, 6, "sal");
	}
	if (eo) {
		eo = x86_dec_shft_k80(bin, bn, addr, 208, 7, "sar");
	}
	if (eo) {
		eo = x86_dec_shft_k81(bin, bn, addr, 209, 0, "rol");
	}
	if (eo) {
		eo = x86_dec_shft_k81(bin, bn, addr, 209, 1, "ror");
	}
	if (eo) {
		eo = x86_dec_shft_k81(bin, bn, addr, 209, 2, "rcl");
	}
	if (eo) {
		eo = x86_dec_shft_k81(bin, bn, addr, 209, 3, "rcr");
	}
	if (eo) {
		eo = x86_dec_shft_k81(bin, bn, addr, 209, 4, "shl");
	}
	if (eo) {
		eo = x86_dec_shft_k81(bin, bn, addr, 209, 5, "shr");
	}
	if (eo) {
		eo = x86_dec_shft_k81(bin, bn, addr, 209, 6, "sal");
	}
	if (eo) {
		eo = x86_dec_shft_k81(bin, bn, addr, 209, 7, "sar");
	}
	if (eo) {
		eo = x86_dec_shft_r8(bin, bn, addr, 210, 0, "rol");
	}
	if (eo) {
		eo = x86_dec_shft_r8(bin, bn, addr, 210, 1, "ror");
	}
	if (eo) {
		eo = x86_dec_shft_r8(bin, bn, addr, 210, 2, "rcl");
	}
	if (eo) {
		eo = x86_dec_shft_r8(bin, bn, addr, 210, 3, "rcr");
	}
	if (eo) {
		eo = x86_dec_shft_r8(bin, bn, addr, 210, 4, "shl");
	}
	if (eo) {
		eo = x86_dec_shft_r8(bin, bn, addr, 210, 5, "shr");
	}
	if (eo) {
		eo = x86_dec_shft_r8(bin, bn, addr, 210, 6, "sal");
	}
	if (eo) {
		eo = x86_dec_shft_r8(bin, bn, addr, 210, 7, "sar");
	}
	if (eo) {
		eo = x86_dec_shft_reg(bin, bn, addr, 211, 0, "rol");
	}
	if (eo) {
		eo = x86_dec_shft_reg(bin, bn, addr, 211, 1, "ror");
	}
	if (eo) {
		eo = x86_dec_shft_reg(bin, bn, addr, 211, 2, "rcl");
	}
	if (eo) {
		eo = x86_dec_shft_reg(bin, bn, addr, 211, 3, "rcr");
	}
	if (eo) {
		eo = x86_dec_shft_reg(bin, bn, addr, 211, 4, "shl");
	}
	if (eo) {
		eo = x86_dec_shft_reg(bin, bn, addr, 211, 5, "shr");
	}
	if (eo) {
		eo = x86_dec_shft_reg(bin, bn, addr, 211, 6, "sal");
	}
	if (eo) {
		eo = x86_dec_shft_reg(bin, bn, addr, 211, 7, "sar");
	}
	if (eo) {
		eo = x86_dec_jmp(bin, bn, addr, 232, "call");
	}
	if (eo) {
		eo = x86_dec_jmp(bin, bn, addr, 233, "jmp");
	}
	if (eo) {
		eo = x86_dec_jmp_byt(bin, bn, addr, 235, "jmp");
	}
	if (eo) {
		eo = x86_dec_byt(bin, bn, addr, 240, "lock");
	}
	if (eo) {
		eo = x86_dec_byt(bin, bn, addr, 242, "repne");
	}
	if (eo) {
		eo = x86_dec_byt(bin, bn, addr, 243, "repe");
	}
	if (eo) {
		eo = x86_dec_byt(bin, bn, addr, 244, "hlt");
	}
	if (eo) {
		eo = x86_dec_byt(bin, bn, addr, 245, "cmc");
	}
	if (eo) {
		eo = x86_dec_alnk(bin, bn, addr, 246, 2, "not");
	}
	if (eo) {
		eo = x86_dec_alnk(bin, bn, addr, 246, 3, "neg");
	}
	if (eo) {
		eo = x86_dec_mlnk(bin, bn, addr, 246, 4, "mul");
	}
	if (eo) {
		eo = x86_dec_mlnk(bin, bn, addr, 246, 5, "imul");
	}
	if (eo) {
		eo = x86_dec_mlnk(bin, bn, addr, 246, 6, "div");
	}
	if (eo) {
		eo = x86_dec_mlnk(bin, bn, addr, 246, 7, "idiv");
	}
	if (eo) {
		eo = x86_dec_byt(bin, bn, addr, 248, "clc");
	}
	if (eo) {
		eo = x86_dec_byt(bin, bn, addr, 249, "stc");
	}
	if (eo) {
		eo = x86_dec_byt(bin, bn, addr, 250, "cli");
	}
	if (eo) {
		eo = x86_dec_byt(bin, bn, addr, 251, "sti");
	}
	if (eo) {
		eo = x86_dec_byt(bin, bn, addr, 252, "cld");
	}
	if (eo) {
		eo = x86_dec_byt(bin, bn, addr, 253, "std");
	}
	if (eo) {
		eo = x86_dec_alnk(bin, bn, addr, 254, 0, "inc");
	}
	if (eo) {
		eo = x86_dec_alnk(bin, bn, addr, 254, 1, "dec");
	}
	if (eo) {
		eo = x86_dec_blnk(bin, bn, addr, 255, 2, "call");
	}
	if (eo) {
		eo = x86_dec_blnk(bin, bn, addr, 255, 4, "jmp");
	}
	if (eo) {
